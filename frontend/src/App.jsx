...
const createDemoResult = () => ({
  ...
  weaknesses: [
    "Some claimed backend depth is not reflected in GitHub activity.",
  ],
  summary:
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-slate-100">
      <div className="absolute inset-0 bg-grid bg-[size:42px_42px] opacity-[0.05]" />
      ...
      <main className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <header className="mx-auto max-w-3xl text-center">
          <div className="inline-flex animate-float items-center rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2 text-sm text-cyan-200 shadow-[0_0_0_1px_rgba(34,211,238,0.08)]">
            Recruiter-grade candidate verification
          </div>
          <h1 className="mt-6 text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:text-6xl">
            AI Resume Truth Checker
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-slate-400 sm:text-lg">
            Verify candidate claims using AI + GitHub validation with a polished screening workflow built for modern
            recruiting teams.
          </p>
        </header>
        ...
        {errorMessage ? (
          <div className="mt-6 rounded-2xl border border-amber-400/20 bg-amber-500/10 px-5 py-4 text-sm text-amber-100 shadow-glow">
            {errorMessage}
          </div>
        ) : null}
